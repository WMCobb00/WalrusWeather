...
public class Controller implements Initializable{
    ...
    public void loadData(){
        ...
        if (city.length()< 1){
            ...
            location.setStyle("-fx-text-box-border: #B22222; -fx-focus-color: #B22222;");
        ...
        try {
            ...
            wallyText.setText("I love "+ idData.title+ "!");
            ...
            switch (scaleSelect.getText()){
                case "Centigrade":
                    ...
                    break;
                case "Fahrenheit":
                    var tempD0F = (((w0Data.the_temp* 1.8)+ 32));
                    var tempD1F = (((w1Data.the_temp* 1.8)+ 32));
                    var tempD2F = (((w2Data.the_temp* 1.8)+ 32));
                    var tempD3F = (((w3Data.the_temp* 1.8)+ 32));
                    var tempD4F = (((w4Data.the_temp* 1.8)+ 32));
                    ...
                    break;
                case "Kelvin":
                    var tempD0K = (w0Data.the_temp+ 273.15);
                    var tempD1K = (w1Data.the_temp+ 273.15);
                    var tempD2K = (w2Data.the_temp+ 273.15);
                    var tempD3K = (w3Data.the_temp+ 273.15);
                    var tempD4K = (w4Data.the_temp+ 273.15);
    ...
    /* Provides a rising animation for weather data */
    @FXML
    private void raiseSlider(){
        DoubleProperty divLoc = slider.getDividers().get(0).positionProperty();

        Timeline tl = new Timeline();
        tl.getKeyFrames().addAll(
                new KeyFrame(Duration.ZERO, new KeyValue(divLoc, 1)),
                new KeyFrame(Duration.millis(725.0d), new KeyValue(divLoc, 0.5d))
        );
        tl.play();
    }

    /* Provides a lowering animation for weather data */
    @FXML
    private void lowerSlider(){
        if (slider.getDividerPositions()[0] < 1) {
            DoubleProperty divLoc = slider.getDividers().get(0).positionProperty();

            Timeline tl = new Timeline();
            tl.getKeyFrames().addAll(
                    new KeyFrame(Duration.ZERO, new KeyValue(divLoc, slider.getDividerPositions()[0])),
                    new KeyFrame(Duration.millis(600.0d), new KeyValue(divLoc, 1))
            );
            tl.play();
        }
    }
    ...
    /* Allows user to execute a query via pressing <ENTER> in the textfield */
    @FXML
    private void onEnter(){
        PauseTransition p = new PauseTransition(Duration.millis(650));
        p.setOnFinished(e -> loadData());
        lowerSlider();
        p.play();
    }
}
